...
class ViewCartPage(Page):
    ...
    def __init__(self, master=None, *args, **kwargs):
        ...
        # configure labelframe grid weights
        ...
        # canvas frame
        canvFrame = tk.Frame(self.label)
        canvFrame.grid_rowconfigure(0, weight=1)
        canvFrame.grid_columnconfigure(0, weight=1)
        canvFrame.grid(row=3, column=0, columnspan=12)

        # Add canv to canvFrame
        canvas = tk.Canvas(canvFrame, bg="yellow")
        ...
        canvas.grid_columnconfigure(4, weight=1)
        canvas.grid_columnconfigure(5, weight=1)
        canvas.grid_columnconfigure(6, weight=1)
        canvas.grid_columnconfigure(7, weight=1)
        canvas.grid_columnconfigure(8, weight=1)
        canvas.grid_columnconfigure(9, weight=1)
        canvas.grid_columnconfigure(10, weight=1)
        canvas.grid_columnconfigure(11, weight=1)

        # Add scrollbar y to canv
        yscrollbar = tk.Scrollbar(
            canvFrame, orient="vertical", command=canvas.yview)
        yscrollbar.grid(row=0, column=1, sticky='ns')
        canvas.configure(yscrollcommand=yscrollbar.set)
        canvas.config(scrollregion=canvas.bbox("all"))

        # add frame inside canvas
        self.contFrame = tk.Frame(canvas, bg="blue")
        canvas.create_window((0, 0), window=self.contFrame, anchor="nw")

        # configure grid weights (resize)
        self.contFrame.grid_columnconfigure(0, weight=1)
        self.contFrame.grid_columnconfigure(1, weight=1)
        self.contFrame.grid_columnconfigure(2, weight=1)
        self.contFrame.grid_columnconfigure(3, weight=1)
        self.contFrame.grid_columnconfigure(4, weight=1)
        self.contFrame.grid_columnconfigure(5, weight=1)
        self.contFrame.grid_columnconfigure(6, weight=1)
        self.contFrame.grid_columnconfigure(7, weight=1)
        self.contFrame.grid_columnconfigure(8, weight=1)
        self.contFrame.grid_columnconfigure(9, weight=1)
        self.contFrame.grid_columnconfigure(10, weight=1)
        self.contFrame.grid_columnconfigure(11, weight=1)
    ...
    def setCart(self):
        ...
        if(len(self.cart) > 0):
            ...
            for box in self.cart:
                # set product image
                boxItem = tk.Label(self.contFrame, image=box.imageSm)
                ...
                # Set Product Name
                name = tk.Label(self.contFrame, text=box.name,
                                font="Helvetica 15 bold")
                name.grid(row=currentRow, column=2, columnspan=3, sticky="w")
                self.cartItems.append(name)

                # Set Product price
                price = tk.Label(self.contFrame, text="Price: $"+"{:.2f}".format(box.price)+"/ea",
                                 font="Helvetica 15")
                ...
                # show addons
                addons = tk.Label(self.contFrame, text=self.getAddOns(
                    box))
                ...
                quant = tk.Label(self.contFrame, text="Quantity: " + str(box.quantity)
                                 )
                ...
                # Remove item button
                removeBtn = tk.Button(
                    self.contFrame, text='Remove', command=lambda: self.removeItemFromCart(box))
                removeBtn.grid(row=currentRow+1, column=9, sticky="e")
                self.cartItems.append(removeBtn)

                # seperator
                sep = ttk.Separator(self.contFrame)
            ...
            # display total
            totalLab = tk.Label(self.contFrame, text="Total: $" +
                                "{:.2f}".format(total))
            ...
            checkoutBtn = tk.Button(
                self.contFrame, text='Checkout', font="Helvetica 18", command=self.ViewCheckoutPageNav)
            checkoutBtn.grid(row=currentRow+1, column=9, sticky="e")
            self.cartItems.append(checkoutBtn)
        else:
            # display empty cart message
            tk.Label(self.contFrame, text='Your cart is empty!', font="Helvetica 25 bold").grid(
                row=3, column=2, columnspan=8, pady=(30, 0))
            tk.Button(self.contFrame, text='Back to Store', width=15,
