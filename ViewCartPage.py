...
class ViewCartPage(Page):
   ...
   def __init__(self,master = None, *args, **kwargs):
       ...
       label = tk.LabelFrame(self, text="This is the View Cart Page")
       ...
       Home = tk.Button(label, text = 'Back to Store', command=self.hide).grid(column = 0, row = 0)
       if(len(self.cart) > 0):
           row = 7
           total = 0.0
           for box in self.cart:
               ...
               boxLabel = tk.Label(label, text = box.description + "\n" + "$" + str(box.price) + "/ea").grid(column = 1, row = row)
               addOnsLabel = tk.Label(label, text = self.getAddOns(box)).grid(column = 1, row = row+1)
               tk.Label(label, text = "Quantity: " + str(box.quantity)).grid(column = 5, row = row,sticky = "e")
               total += self.getBoxTotal(box)
               row+=2
           tk.Label(label, text = "Total: $" + "{:.2f}".format(total)).grid(column = 5, row = row,sticky = "e")
           Checkout = tk.Button(label, text = 'Checkout', command= self.ViewCheckoutPageNav).grid(row = row+1, column = 5, sticky = "e")

       else:
           tk.Label(label, text = 'Cart is empty!').grid(row = 7, column = 2, columnspan = 5)
